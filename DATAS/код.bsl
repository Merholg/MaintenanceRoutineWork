...
//&НаСервере
//Процедура ФорматЗаписиПериода(ВхДата)
//	Если (ВхДата = Неопределено) ИЛИ (Год(ВхДата) < 1915) Тогда 
//		ВхДата = НачалоГода(ТекущаяДата());
//	ИначеЕсли ВхДата <> НачалоГода(ВхДата) Тогда 
//		ВхДата = НачалоГода(ВхДата);
//	КонецЕсли;
//КонецПроцедуры	

//&НаСервере
//Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
//	ФорматЗаписиПериода(Запись.Период);
//КонецПроцедуры

//&НаСервере
//Процедура ПриЧтенииНаСервере(ТекущийОбъект)
//	ФорматЗаписиПериода(Запись.Период);
//	Если Запись.Работы <> Неопределено Тогда 
//		ЭтаФорма.реквСистемаРабот = Запись.Работы.Система;
//		ЭтаФорма.реквПолноеНаименованиеРабот = Запись.Работы.ПолноеНаименование;
//	КонецЕсли;
//	Если Запись.Конструкции <> Неопределено Тогда
//		ЭтаФорма.реквИерарх = ПоискИерарха(Запись.Конструкции);
//	КонецЕсли;
//КонецПроцедуры
